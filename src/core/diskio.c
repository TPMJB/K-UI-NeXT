...
DSTATUS disk_initialize(BYTE drive) {
    ...
    attempted = true;
    if(!media.blocks || !media.read || !media.write || !media.sync) return state;
    uint8_t sector[512];
    uint64_t blocks = media.blocks(media.ctx);
    if(!blocks || blocks > UINT32_MAX || media.read(media.ctx, 0, 1, sector) ||
       !kui_select_volume(sector, blocks, &volume)) return state;
    state = 0;
    return state;
}
