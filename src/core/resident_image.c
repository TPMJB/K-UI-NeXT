...
#define EXTENT_BASE (TRACK_BASE + KUI_GAME_TRACK_MAX * TRACK_BYTES)
#define SAMPLE_BASE (EXTENT_BASE + KUI_RESIDENT_IMAGE_EXTENTS * 12u)
#define USED_BYTES (SAMPLE_BASE + KUI_RESIDENT_IMAGE_SAMPLES * 16u)
_Static_assert(USED_BYTES <= KUI_RESIDENT_IMAGE_WIRE_BYTES, "manifest wire capacity");
_Static_assert(sizeof(struct kui_resident_manifest) < 65536u, "bounded manifest");
...
uint32_t kui_resident_crc32(uint32_t previous, const void *data, size_t bytes) {
...
static uint32_t wire_crc(const uint8_t *wire) {
    ...
    uint32_t crc = kui_resident_crc32(0, wire, 16);
    crc = kui_resident_crc32(crc, zero, 4);
    return kui_resident_crc32(crc, wire + 20,
                              KUI_RESIDENT_IMAGE_WIRE_BYTES - 20u);
}
...
static bool tracks_valid(const struct kui_resident_manifest *m) {
    if(!m || !m->track_count || m->track_count > KUI_GAME_TRACK_MAX)
        return false;
    for(uint32_t i = 0; i < m->track_count; ++i) {
        const struct kui_resident_track *t = &m->tracks[i];
...
static enum kui_game_result range_check(const struct kui_resident_manifest *m,
...
enum kui_game_result kui_resident_image_check(const struct kui_resident_manifest *m,
    uint32_t lba, uint32_t count, enum kui_game_sector_format format) {
    if(!tracks_valid(m)) return KUI_GAME_INVALID;
    if(count > KUI_RESIDENT_IMAGE_MAX_SECTORS) return KUI_GAME_RANGE;
    return range_check(m, lba, count, format);
}
enum kui_game_result kui_resident_manifest_validate(const struct kui_resident_manifest *m) {
    if(!tracks_valid(m) || !m->card_sectors || m->card_sectors > UINT64_C(0x100000000) ||
       ...
       !m->extent_count || m->extent_count > KUI_RESIDENT_IMAGE_EXTENTS ||
       !m->sample_count || m->sample_count > KUI_RESIDENT_IMAGE_SAMPLES ||
    ...
    for(uint32_t i = 0; i < m->track_count; ++i) {
        ...
        for(uint32_t j = 0; j < t->extent_count; ++j) {
            const struct kui_resident_extent *e = &m->extents[next_extent + j];
    ...
    if(next_extent != m->extent_count || !session_found || !m->boot_bytes ||
       m->boot_bytes > 16u * 1024u * 1024u || m->boot_lba < m->session_lba)
        return KUI_GAME_INVALID;
    ...
    for(uint32_t i = 0; i < m->extent_count; ++i) {
        const struct kui_resident_extent *e = &m->extents[i];
        for(uint32_t j = 0; j < i; ++j) {
            const struct kui_resident_extent *q = &m->extents[j];
    ...
    for(uint32_t i = 0; i < m->sample_count; ++i) {
        const struct kui_resident_sample *s = &m->samples[i];
        r = kui_resident_image_check(m, s->lba, s->count,
                                     (enum kui_game_sector_format)s->format);
        if(r != KUI_GAME_OK) return r;
    }
    return KUI_GAME_OK;
}
enum kui_game_result kui_resident_manifest_encode(const struct kui_resident_manifest *m,
    uint8_t out[KUI_RESIDENT_IMAGE_WIRE_BYTES]) {
    if(!out) return KUI_GAME_INVALID;
    enum kui_game_result r = kui_resident_manifest_validate(m);
    if(r != KUI_GAME_OK) return r;
    memset(out, 0, KUI_RESIDENT_IMAGE_WIRE_BYTES);
    memcpy(out, "KUIG4I01", 8);
    put32(out + 8, KUI_RESIDENT_IMAGE_VERSION);
    put32(out + 12, KUI_RESIDENT_IMAGE_WIRE_BYTES);
    put32(out + 20, m->track_count); put32(out + 24, m->extent_count);
    put32(out + 28, m->sample_count); put64(out + 32, m->card_sectors);
    ...
    for(uint32_t i = 0; i < m->sample_count; ++i) {
        uint8_t *p = out + SAMPLE_BASE + i * 16;
        put32(p, m->samples[i].lba); put32(p + 4, m->samples[i].count);
        put32(p + 8, m->samples[i].format); put32(p + 12, m->samples[i].crc32);
    }
    put32(out + 16, wire_crc(out));
    return KUI_GAME_OK;
}
enum kui_game_result kui_resident_manifest_decode(
    const uint8_t wire[KUI_RESIDENT_IMAGE_WIRE_BYTES], struct kui_resident_manifest *m) {
    if(!m) return KUI_GAME_INVALID;
    memset(m, 0, sizeof(*m));
    if(!wire || memcmp(wire, "KUIG4I01", 8) ||
       get32(wire + 8) != KUI_RESIDENT_IMAGE_VERSION ||
       get32(wire + 12) != KUI_RESIDENT_IMAGE_WIRE_BYTES ||
       get32(wire + 16) != wire_crc(wire) || !zeroes(wire + 256, 64) ||
       !zeroes(wire + USED_BYTES, KUI_RESIDENT_IMAGE_WIRE_BYTES - USED_BYTES))
        return KUI_GAME_INVALID;
    m->track_count = get32(wire + 20); m->extent_count = get32(wire + 24);
    m->sample_count = get32(wire + 28);
    if(m->track_count > KUI_GAME_TRACK_MAX ||
       m->extent_count > KUI_RESIDENT_IMAGE_EXTENTS ||
       m->sample_count > KUI_RESIDENT_IMAGE_SAMPLES) goto invalid;
    ...
    for(uint32_t i = 0; i < KUI_GAME_TRACK_MAX; ++i) {
        ...
        struct kui_resident_track *t = &m->tracks[i];
    ...
    for(uint32_t i = 0; i < KUI_RESIDENT_IMAGE_EXTENTS; ++i) {
    ...
    for(uint32_t i = 0; i < KUI_RESIDENT_IMAGE_SAMPLES; ++i) {
        const uint8_t *p = wire + SAMPLE_BASE + i * 16;
        if(i >= m->sample_count) { if(!zeroes(p, 16)) goto invalid; continue; }
        m->samples[i].lba = get32(p); m->samples[i].count = get32(p + 4);
        m->samples[i].format = get32(p + 8); m->samples[i].crc32 = get32(p + 12);
    }
    enum kui_game_result r = kui_resident_manifest_validate(m);
...
enum kui_game_result kui_resident_image_init(struct kui_resident_image *image,
    const struct kui_resident_manifest *manifest, kui_resident_read_block read, void *context) {
    if(!image) return KUI_GAME_INVALID;
    memset(image, 0, sizeof(*image));
    if(!read) return KUI_GAME_INVALID;
    enum kui_game_result r = kui_resident_manifest_validate(manifest);
    if(r != KUI_GAME_OK) return r;
    image->manifest = manifest; image->read_block = read; image->context = context;
    return KUI_GAME_OK;
}
static enum kui_game_result file_read(struct kui_resident_image *image,
    const struct kui_resident_track *track, uint32_t offset, uint8_t *out, uint32_t bytes) {
    const struct kui_resident_manifest *m = image->manifest;
    ...
    while(bytes) {
        ...
        const struct kui_resident_extent *e = &m->extents[lo];
...
enum kui_game_result kui_resident_image_read(struct kui_resident_image *image,
    ...
    enum kui_game_result r = kui_resident_image_check(image->manifest, lba, count, format);
    if(r != KUI_GAME_OK) return r;
    ...
    for(uint32_t i = 0; i < count; ++i) {
        ...
        const struct kui_resident_track *t = &image->manifest->tracks[track_index];
        r = file_read(image, t, (current - t->start_lba) * KUI_GAME_RAW_BYTES,
                      image->raw, KUI_GAME_RAW_BYTES);
        if(r != KUI_GAME_OK) return r;
        uint32_t inside = 0;
        if(format == KUI_GAME_SECTOR_MODE1) {
            if(image->raw[0] || image->raw[11] || image->raw[15] != 1) return KUI_GAME_MODE;
            for(unsigned j = 1; j < 11; ++j) if(image->raw[j] != 255) return KUI_GAME_MODE;
            inside = 16;
        }
        memcpy(destination, image->raw + inside, sector_bytes);
