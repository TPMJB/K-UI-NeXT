...
bool kui_system_settings_encode(uint8_t out[KUI_SYSTEM_SETTINGS_RECORD_SIZE],
    ...
    memcpy(out,magic,sizeof(magic));put32(out+8,3);put32(out+12,KUI_SYSTEM_SETTINGS_RECORD_SIZE);
    ...
    out[25]=(uint8_t)((settings->show_memory?1u:0u)|(settings->music_enabled?2u:0u)|
                      (settings->startup_chime?4u:0u)|(settings->menu_sounds?8u:0u));
    out[26]=(uint8_t)settings->music_volume;
    out[27]=(uint8_t)settings->startup_app;
    out[28]=(uint8_t)settings->screen_inset;
...
bool kui_system_settings_decode(struct kui_system_settings *settings,uint64_t *sequence,
    ...
    if(memcmp(p,magic,sizeof(magic)) || (version<1 || version>3) ||
       get32(p+12)!=KUI_SYSTEM_SETTINGS_RECORD_SIZE || !get64(p+16) ||
       (p[25]&~(version==1?3u:version==2?7u:15u)) || get32(p+36)!=kui_crc32(0,p,36)) return false;
    for(unsigned i=version==1?27:version==2?28:29;i<36;i++) if(p[i]) return false;
    ...
    if(version>=3) {decoded.screen_inset=p[28];decoded.menu_sounds=(p[25]&8u)!=0;}
