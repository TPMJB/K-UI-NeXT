...
void kui_loader_display_init(void) {
    ...
    kui_shell_font_draw(frame, 20, 16, 0x7fff, "K-UI: selected-image GD probe", true);
    kui_shell_font_draw(frame, 20, 47, 0xffff, "Build " KUI_BUILD_ID, false);
    kui_shell_font_draw(frame, 20, 70, 0xffff,
        "BIOS vector / selected GDI / no KOS", false);
    row = 112;
}
void kui_image_display_title(const char *title) {
    char visible[76];
    unsigned n = 0;
    while(title[n] && n < sizeof(visible) - 1) {
        visible[n] = title[n];
        n++;
    }
    visible[n] = 0;
    for(unsigned y = 70; y < 90; y++)
        for(unsigned x = 16; x < 624; x++) frame[y * WIDTH + x] = BACKGROUND;
    kui_shell_font_draw(frame, 20, 70, 0xffff, visible, false);
}
