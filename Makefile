...
CAPTURE = src/core/hash.c src/core/capture_plan.c src/core/capture.c
FATFS = .deps/fatfs/source/ff.c .deps/fatfs/source/ffunicode.c

.PHONY: test test-images deps diagnostic clean
test: build/test-core build/test-capture-core
	./build/test-core
...
build/capture-image: tests/capture_image.c $(CORE) $(CAPTURE) src/core/storage_probe.c $(FATFS) include/kui/capture.h
	@mkdir -p build
