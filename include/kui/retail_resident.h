 ...
 * entry clears resident BSS. Every input is copied before returning, and the
 * BIOS vector is changed only on complete success. There is no high-stage
 * pointer retained after success. */
typedef int (*kui_retail_resident_entry)(const uint8_t wire[KUI_RETAIL_MAP_BYTES],
    uint32_t original_gd_vector, const struct retail_display_state *display);
