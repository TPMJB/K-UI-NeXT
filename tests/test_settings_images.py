...
"""Exercise settings recovery and options overlays on real FAT32/exFAT images."""
...
BINARY = str(ROOT / "build/settings-image")
...
def main():
    with tempfile.TemporaryDirectory(prefix="kui-settings-") as temp:
        base = Path(temp)
        for kind in ("fat32", "exfat"):
            ...
            for case in ("alternate", "corrupt", "truncated", "oversize", "version", "flags",
                         "both-invalid", "options", "overflow", "read-fail", "save-read-fail", "write-fail", "sync-fail",
                         "readback-fail", "readback-corrupt"):
                ...
                assert f"PASS settings {case}" in output
                ...
                print(f"PASS {kind} settings: {case}, capture files preserved", flush=True)
