...
static void same(struct kui_system_settings a,struct kui_system_settings b) {
    assert(a.video_mode==b.video_mode && a.show_memory==b.show_memory &&
           ...
           a.startup_chime==b.startup_chime && a.startup_app==b.startup_app);
}
...
static void rejects(const void *record,size_t size) {
    struct kui_system_settings before={KUI_VIDEO_PAL50,false,true,100,false,KUI_STARTUP_MUSIC},out=before;
...
int main(void) {
    ...
    /* Independent Python struct.pack/zlib fixtures: same defaults in both
     * formats. New fields are defaults when reading the older record. */
    const uint8_t fixtures[2][KUI_SYSTEM_SETTINGS_RECORD_SIZE]={
    ...
    assert(kui_system_settings_encode(record,&defaults,1));assert(!memcmp(record,fixtures[1],sizeof(fixtures[1])));
    for(unsigned version=0;version<2;version++) {
        ...
        for(unsigned i=0;i<sizeof(bytes)/sizeof(bytes[0]);i++) {
            ...
            record[pos]=pos==24?KUI_VIDEO_MODE_COUNT:pos==25?8:pos==26?101:pos==16?0:
                        pos==8?3:pos==27?KUI_STARTUP_APP_COUNT:(uint8_t)(record[pos]+1);
    ...
    for(unsigned mode=0;mode<KUI_VIDEO_MODE_COUNT;mode++) for(unsigned flags=0;flags<8;flags++)
        for(unsigned app=0;app<KUI_STARTUP_APP_COUNT;app++) for(unsigned volume=0;volume<=100;volume++)
            for(unsigned i=0;i<3;i++) {
                struct kui_system_settings wanted={mode,(flags&1u)!=0,(flags&2u)!=0,volume,(flags&4u)!=0,app};
    ...
    for(unsigned mode=0;mode<KUI_VIDEO_MODE_COUNT;mode++) for(unsigned flags=0;flags<4;flags++) {
        ...
        assert(kui_system_settings_decode(&out,&sequence,record,KUI_SYSTEM_SETTINGS_RECORD_SIZE));
        struct kui_system_settings migrated={mode,(flags&1u)!=0,(flags&2u)!=0,10,true,KUI_STARTUP_HOME};
        same(out,migrated);
    }
    ...
    puts("PASS system settings: v1 migration, v2 fixtures, bounded fields, reserved bytes, CRC and sequences");
    return 0;
}
