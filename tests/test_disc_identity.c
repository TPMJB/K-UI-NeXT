...
static struct {
    ...
    unsigned polls,prepares,reads,phases;
...
bool kui_disc_prepare(struct kui_toc sessions[2]) {
    ...
    if(fake.status==CD_STATUS_RETRY) fake.status=CD_STATUS_PAUSED;
    return true;
}
...
int main(void) {
    ...
    assert(id.state==KUI_DISC_IDENTITY_RESET_REQUIRED);
    polls=fake.polls;kui_disc_identity_invalidate(&id);
    fake.status=CD_STATUS_OPEN;
    assert(!kui_disc_identity_poll(&id,ops,500,true) && fake.polls==polls);
    ...
    puts("PASS idle disc status: no calls while busy, bounded polling, one read per insertion, title EDC/signature, CD exclusion, failure/cancel/lid-open, poisoned-adapter preservation");
    return 0;
}
